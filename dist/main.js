...
async function storeMessages(convoId, messages) {
...
function search(mini, query, msgCache, opts = {}) {
...
function filterBySender(results, senderDid) {
  return results.filter((r) => r.msg && r.msg.senderDid === senderDid);
}
function filterByTime(results, start, end) {
...
var ChatSearchPage = class {
  ...
  constructor(plugin) {
    ...
    this.msgCache = /* @__PURE__ */ new Map();
    this.container = null;
  }
  async init() {
    ...
    const template = html`
      <div class="cs-page">
        ...
        <div class="cs-pull-bar">
          <select class="cs-select" .value=${this.selectedConvoId} @change=${(e) => {
      this.selectedConvoId = e.target.value;
      this.#render();
    }}>
          ...
          ${this.pulling ? html`<span class="cs-progress">Fetched: ${this.pullProgress.fetched}${this.pullProgress.total != null ? " / " + this.pullProgress.total : "\u2026"}</span>` : null}
        </div>
      ...
      if (e.key === "Enter") this.handleSearch();
          ...
          <div class="cs-filters">
            <select class="cs-select" style="min-width:140px" .value=${this.senderFilter} @change=${(e) => {
      this.senderFilter = e.target.value;
      this.handleSearch();
    }}>
        ...
        this.handleSearch();
            ...
            <label style="font-size:12px;margin-left:8px">
              ...
              <input type="range" min="0" max="0.4" step="0.05" .value=${this.fuzzyLevel}
                @input=${(e) => {
      this.fuzzyLevel = Number(e.target.value);
    }} />
            </label>
          </div>

          ${this.searched && this.results.length === 0 && !this.searching ? html`<p class="cs-empty">No matching messages found.</p>` : null}
...
// src/sidebar.js
function registerSidebar(plugin) {
  try {
...
// src/main.js
var ChatSearchPlugin = class extends Plugin {
  async onload() {
    this.db = await openDB();
    registerSidebar(this);
    this.#registerPage();
    this.fetchConvoList = () => this.callHost("getConvoList");
    this.fetchConvoMessages = (convoId, cursor) => this.callHost("getConvoMessages", { convoId, cursor });
  ...
  #registerPage() {
    try {
      if (this.addRegistrationTarget) {
        ...
        console.warn(
          "[chat-search] Could not determine page registration API. The sidebar item will still work, but you may need to manually wire the full-page route."
        );
      }
    } catch (err) {
